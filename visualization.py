...
def vis_vol_cluster(objects, highp, window, N_time, image_dir = None):
    ...
    for i in range(len(objects)):
        ...
        if len(count_herd) > 0:
            ...
            plt.plot(bin_count_herd[0], mean_count_herd, label=f"Herd model, {object[1][1]} agents, R = {round(mean_measure_herd,2)} $\\pm$ {round(conf_herd, 2)}")
            # plt.plot(bin_count_herd[0], mean_count_herd, label=f"Herd model, Pa = {object[1][2]}, R = {round(mean_measure_herd,2)} $\\pm$ {round(conf_herd, 2)}")
...
def visualiseMultipleMarketResults(N_agents, N_time, C, A, p, garch, garch_n, garch_param, Pa, Pc, cluster, N):
    ...
    # Image directory
    image_dir = f'images/Nagents{N_agents}_Pa{Pa}_Pc{Pc}_cluster'
